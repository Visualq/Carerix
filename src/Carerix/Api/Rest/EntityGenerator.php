...
class Carerix_Api_Rest_EntityGenerator {
    ...
    protected function addAttribute(Zend_CodeGenerator_Php_Class &$class, SimpleXMLElement $attribute) {
        ...
        $classTagSetter =  array(
               'name' => 'method',
            'description'  => $class->getName().' '.$setMethodName.'() '.$setMethodName.'('.$paramType.' $value) Set '.$attribute['name'].' attribute. Provides a fluent interface.',
        );
        $classTagGetter = array(
            'name' => 'method',
            'description'  => $paramType.' '.$getMethodName.' '.$getMethodName.'() Get '.$attribute['name'].' attribute.',
        );
        
        $classTags = array($classTagSetter, $classTagGetter);
...
//         }
        ...
        $classTagSetter = array(
            'name' => 'method',
            'description'  => $class->getName().' '.$setMethodName.'() '.$setMethodName.'('.(self::isMany($relationship) ? 'array|'.self::COLLECTION_CLASS.' $values' : 'array|'.$paramType.' $value').') Set '.$relationship['name'].' relationship. Provides a fluent interface.',
        );        
        $classTagGetter = array(
            'name' => 'method',
            'description'  => (self::isMany($relationship) ? self::COLLECTION_CLASS.'|'.$paramType : $paramType).' '.$getMethodName.' '.$getMethodName.'(%s) Get '.$relationship['name'].' relationship.',
        );
    ...
    public static function getClassDocblock() {
        return array(
            'tags' => array(
                ...
                array(
                    'name' => 'copyright',
                    'description'  => 'Copyright 2011-'.date('Y').' Carerix (http://www.carerix.com). All rights reserved.',
                ),
                array(
                    'name' => 'author',
